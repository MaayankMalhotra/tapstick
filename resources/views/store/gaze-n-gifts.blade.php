...
<body class="bg-parchment text-espresso">
    ...
    <script>
        ...
        @if(!empty($grazeMenuCategoriesJson))
        (function() {
            try {
                const dynamicCategories = {!! $grazeMenuCategoriesJson !!};
                const indoService = servicesData.find(s => s.id === 'indo-fusion');
                if (indoService && indoService.fullMenu && Array.isArray(dynamicCategories) && dynamicCategories.length > 0) {
                    indoService.fullMenu.categories = dynamicCategories;
                }
            } catch (e) {
                console.error('Failed to parse dynamic graze menu categories:', e);
            }
        })();
        @endif
